...
static uint8 gps_gnrmc_parse (char *line, gps_info_struct *gps)
{
    ...
    if('A' == state)                                                            // 如果数据有效 则解析数据
    {
		...
		gps->latitude_minute    = lati_cent_tmp;
		...
		gps->longitude_minute    = long_cent_tmp;
...
double x = 0;
	...
	{
		if (y < 0 && angle > 0) angle -= 180.0f;
		else if (y < 0 && angle < 0) angle += 180.0f;
	}
...
//-------------------------------------------------------------------------------------------------------------------
void gps_uart_callback (uint8 uart_dat)
{
    uint8 temp_gps[6];
    ...
    {
		fifo_write_buffer(&gps_tau1201_receiver_fifo, &uart_dat, 1);
        if('\n' == uart_dat)
        {
            // 读取前6个数据 用于判断语句类型
            temp_length = 6;
...
void gps_init (void)
{
    ...
    delay_ms(500);                                                           // 等待GPS启动后开始初始化

	uart_init(GPS_TAU1201_UART, GPS_TAU1201_RX, GPS_TAU1201_TX, 115200, GPS_TAU1201_TIMER_N);	//初始化串口    
